...
interface_bl = { "ut-0/0/0","pfh-0/0/0","pfe-0/0/0","pd-0/0/0","ip-0/0/0","lt-0/0/0","lc-0/0/0","ud-0/0/0","vt-0/0/0","mt-0/0/0","pe-0/0/0","lsi","em0","lo0","pime","esi",".local.","pimd","tap","gre","demux0","ipip","pip0","irb","dsc","mtun","vtep","em1","cbp0","jsrv","pp0", "bme0" }
...
def ipaddress_lookup(ip,device):
     ...
     if json["count"] == 1:
       ...
       raise Exception('The mentioned ip address does not exist on this device or returned more than 1 result')
...
# Add IP address # TODO Check if in use or assigned to something else
          if interface_lookup(interface,device):
            ...
            if not ipaddress_lookup(data['address'],device):
              if not ipaddress_add(data['address'],data['interface'],data['vrf'],data['status']):
